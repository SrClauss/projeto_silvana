...
const CriarVenda: React.FC = () => {
  ...
  const addItem = () => {
    ...
    setItensVenda(prev => [...prev, newItem]);
  };
  ...
  const updateItem = (id: string, updates: Partial<ItemVenda>) => {
    setItensVenda(prev => prev.map(item => item.id === id ? { ...item, ...updates } : item));
  };

  const openEditModal = (item: ItemVenda) => {
    setEditingItem(item);
  ...
  const saveEditModal = () => {
    if (editingItem) {
      const preco = editingItem.produto?.preco_venda || 0;
      const valorTotal = editingItem.quantidade * preco;
      updateItem(editingItem.id, { quantidade: editingItem.quantidade, observacao: editingItem.observacao, valorTotal });
    }
  ...
  useEffect(() => {
    ...
    return () => { if (produtoSearchTimer.current) window.clearTimeout(produtoSearchTimer.current); };
  ...
  const handleSubmit = async () => {
    setError(null);
    if (itensVenda.length === 0) return setError('Adicione pelo menos um produto');
    ...
    setLoading(true);
    try {
      const vendas = itensVenda.map(item => {
        let obs = '';
        if (item.observacao.trim()) {
          obs += `Observação Específica : ${item.observacao.trim()}\n`;
        }
        if (observacaoGeral.trim()) {
          obs += `Observação geral : ${observacaoGeral.trim()}`;
        }
        return {
      ...
      await api.post('/vendas/batch', vendas);
      navigate('/vendas');
    } catch (e: unknown) {
      console.error(e);
      const msg = e instanceof Error ? e.message : String(e);
      setError(msg || 'Erro ao criar vendas');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Box sx={{ width: '100%', maxWidth: 700 }}>
        ...
        <Paper sx={{ p: 2, mb: 2 }}>
          ...
          {itensVenda.map((item) => (
            <Paper key={item.id} sx={{ p: 2, mb: 2, border: '1px solid #ddd' }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                <Typography variant="subtitle2">Produto {itensVenda.indexOf(item) + 1}</Typography>
              ...
              {item.produto ? (
                <Box>
                  ...
                  <Typography variant="body2">Valor Total: {formatCurrency(item.valorTotal)}</Typography>
          ...
          <Tooltip title="Adicionar Produto">
            <IconButton onClick={addItem} sx={{ mb: 2 }}>
              <AddIcon />
            </IconButton>
          </Tooltip>
        </Paper>
        ...
        <Paper sx={{ p: 3, mb: 2, borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            ...
            <Box sx={{ textAlign: 'right' }}>
              ...
              <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                {itensVenda.reduce((sum, item) => sum + item.quantidade, 0)} unidades
              </Typography>
            </Box>
          </Box>
          ...
          <Box sx={{ textAlign: 'center' }}>
            <Typography variant="h5">
              Total da Venda: {formatCurrency(itensVenda.reduce((sum, item) => sum + item.valorTotal, 0))}
            </Typography>
          </Box>
          ...
          <Typography variant="body2" sx={{ textAlign: 'center', color: 'text.secondary' }}>
            {itensVenda.filter(item => item.produto).length} produto{itensVenda.filter(item => item.produto).length !== 1 ? 's' : ''} selecionado{itensVenda.filter(item => item.produto).length !== 1 ? 's' : ''}
          </Typography>
        </Paper>
      </Box>
